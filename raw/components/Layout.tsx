  ...
  return (
    <div className={`min-h-screen flex ${isDarkMode ? 'dark bg-slate-950 text-white' : 'bg-slate-50 text-slate-900'}`}>
      <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-slate-900 border-r border-slate-200 dark:border-slate-800/50 transform transition-transform duration-300 lg:translate-x-0 ${isMobileMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="absolute bottom-0 w-full p-6 border-t border-slate-100 dark:border-slate-800 space-y-4">
          {/* Dynamic Plan Capacity Widget */}
          <div className="p-4 rounded-2xl bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-700/50">
            ...
            <div className="flex justify-between items-center mt-2">
               <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">
                {aiUsage} / {aiLimit} AI Post Usages
               </p>
            ...
            <div className="flex justify-between items-center mt-2">
               <p className="text-[8px] font-bold text-slate-400 uppercase tracking-widest">
                {imageUsage} / {imageLimit} AI Image Usages
               </p>
            </div>
               {imageUsagePercentage > 80 && user?.role === UserRole.USER || user?.role === UserRole.ADMIN && (
                 <button onClick={() => navigate('#/admin/plans')} className="text-[8px] font-black text-blue-600 uppercase flex items-center gap-1 hover:underline">
                    Upgrade <ArrowUpCircle className="w-2.5 h-2.5" />
                 </button>
               )}
          ...
          {user?.role === UserRole.USER || user?.role === UserRole.ADMIN && (
            <div className={`p-4 rounded-2xl border transition-all ${user.linkedInConnected ? 'bg-blue-50 dark:bg-blue-900/10 border-blue-100 dark:border-blue-900' : 'bg-red-50 dark:bg-red-900/10 border-red-100 dark:border-red-900'}`}>
               <div className="flex items-center gap-3">
                  ...
                  <div onClick={() => navigate('#/settings')}  className="overflow-hidden">
      ...
      <main className="flex-1 lg:ml-64 flex flex-col min-h-screen relative overflow-hidden">
        <header className="h-20 bg-white/80 dark:bg-slate-900/80 backdrop-blur-md border-b border-slate-200 dark:border-slate-800 flex items-center justify-between px-8 sticky top-0 z-40">
          ...
          <div className="flex items-center gap-6">
            <div className="hidden sm:flex items-center gap-2 px-3 py-1.5 bg-slate-100 dark:bg-slate-800 rounded-full border dark:border-slate-700">
               <ShieldCheck className="w-3.5 h-3.5 text-green-500" />
               <span className="text-[9px] font-black uppercase tracking-widest text-slate-500">Security: Encrypted</span>
            </div>
